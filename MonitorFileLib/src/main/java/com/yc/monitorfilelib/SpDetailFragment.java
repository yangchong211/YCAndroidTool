...
public class SpDetailFragment extends Fragment {
    ...
    private List<SpDataBean> getSpBeans() {
        ...
        if (data != null && mActivity != null) {
            ...
            Map<String, ?> all = AppFileSpUtils.getInstance(spTableName).getAll();
            if (all.isEmpty()) {
                return spBeans;
            }
    ...
    public void spUpData(SpDataBean bean) {
        ...
        switch (bean.value.getClass().getSimpleName()) {
            case SpInputType.STRING:
                AppFileSpUtils.getInstance().put(key, bean.value.toString());
                break;
            case SpInputType.BOOLEAN:
                AppFileSpUtils.getInstance().put(key, (boolean) bean.value);
                break;
            case SpInputType.INTEGER:
                AppFileSpUtils.getInstance().put(key, (int) bean.value);
                break;
            case SpInputType.FLOAT:
                AppFileSpUtils.getInstance().put(key, (float) bean.value);
                break;
            case SpInputType.LONG:
                AppFileSpUtils.getInstance().put(key, (long) bean.value);
                break;
            default:
                break;
        }
