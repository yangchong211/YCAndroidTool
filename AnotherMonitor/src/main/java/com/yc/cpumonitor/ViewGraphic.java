...
//		canvas.drawPath(graphicPath, linesGridPaint);
		...
		if (l != null && !l.isEmpty())
			for (int n=0; n<l.size(); ++n) {
				if ((Boolean) l.get(n).get(Constants.pSelected)) {
					if (paints == null)
						paints = new HashMap<String, Paint>();
					Paint paint = paints.get(l.get(n).get(Constants.pId));
					if (paint == null) {
						paint = getPaint((Integer) l.get(n).get(Constants.pColour), Paint.Align.CENTER, 12, false, thickParam);
						paints.put((String) l.get(n).get(Constants.pId), paint);
					}
					if (processesMode == Constants.processesModeShowCPU) // processesMode==0 CPU usage, processesMode==1 Memory
						drawLineFloat((List<Float>) l.get(n).get(Constants.pFinalValue), canvas, paint);
					else drawLineInteger((List<Integer>) l.get(n).get(Constants.pTPD), canvas, paint);
				}
			}
		
		if (graphicMode == Constants.graphicModeShowMemory) {
			if (memUsedD)
