...
/*		public synchronized void stop() {
	...
	private volatile Thread readThread = new Thread(readRunnable, Constants.readThread);
	...
	receiverClose = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			sendBroadcast(new Intent(Intent.ACTION_CLOSE_SYSTEM_DIALOGS));
			sendBroadcast(new Intent(Constants.actionFinishActivity));
			stopSelf();
		}
	};
...
//		LocalBroadcastManager.getInstance(this).registerReceiver(receiver, new IntentFilter(Constants.anotherMonitorEvent));
		registerReceiver(receiverStartRecord, new IntentFilter(Constants.actionStartRecord));
		registerReceiver(receiverStopRecord, new IntentFilter(Constants.actionStopRecord));
		registerReceiver(receiverClose, new IntentFilter(Constants.actionClose));
...
//				.addNextIntent(new Intent(this, ActivityMain.class))
				.addNextIntentWithParentStack(new Intent(this, CpuMonitorActivity.class))
				.getPendingIntent(0, PendingIntent.FLAG_UPDATE_CURRENT);
		PendingIntent pIStartRecord = PendingIntent.getBroadcast(this, 0, new Intent(Constants.actionStartRecord), PendingIntent.FLAG_UPDATE_CURRENT);
		PendingIntent pIStopRecord = PendingIntent.getBroadcast(this, 0, new Intent(Constants.actionStopRecord), PendingIntent.FLAG_UPDATE_CURRENT);
		PendingIntent pIClose = PendingIntent.getBroadcast(this, 0, new Intent(Constants.actionClose), PendingIntent.FLAG_UPDATE_CURRENT);
		...
		try {
			...
			while (s != null) {
				...
				if (mListSelected != null && !mListSelected.isEmpty()) {
					List<Integer> l = (List<Integer>) (mListSelected.get(0)).get(Constants.pFinalValue);
					if (l != null && l.size() >= maxSamples)
						for (Map<String, Object> m : mListSelected) {
							((List<Integer>) m.get(Constants.pFinalValue)).remove(l.size() - 1);
					...
					for (Map<String, Object> p : mListSelected) {
						try {
							if (p.get(Constants.pDead) == null) {
								...
								arrayPIds[n] = Integer.valueOf((String) p.get(Constants.pId));
								++n;
								sa = reader.readLine().split("[ ]+", 18);
								p.put(Constants.work, (float) Long.parseLong(sa[13]) + Long.parseLong(sa[14]) + Long.parseLong(sa[15]) + Long.parseLong(sa[16]));
								reader.close();
							}
	...
	private void record() {
		...
		try {
			if (topRow) {
				...
				if (mListSelected != null && !mListSelected.isEmpty())
					for (Map<String, Object> p : mListSelected)
						sb.append(",").append(p.get(Constants.pAppName)).append(" (Pid ").append(p.get(Constants.pId)).append(") CPU usage (%)")
						  .append(",").append(p.get(Constants.pAppName)).append(" Memory (kB)");
			...
			if (mListSelected != null && !mListSelected.isEmpty())
				for (Map<String, Object> p : mListSelected) {
					if (p.get(Constants.pDead) != null)
						sb.append(",DEAD,DEAD");
					else sb.append(",").append(((List<Integer>) p.get(Constants.pFinalValue)).get(0))
							.append(",").append(((List<Integer>) p.get(Constants.pTPD)).get(0));
				}
			sb.append(",")
...
//			MediaScannerConnection.scanFile(this, new String[] { mFile.getAbsolutePath() }, null, null);
	...
	void removeProcess(Map<String, Object> process) {
		synchronized (mListSelected) {
			...
			while (i.hasNext())
				if (i.next().get(Constants.pId).equals(process.get(Constants.pId))) {
					i.remove();
					Log.i(getString(R.string.w_processes_dead_notification), (String) process.get(Constants.pName));
				}
		}
	}
