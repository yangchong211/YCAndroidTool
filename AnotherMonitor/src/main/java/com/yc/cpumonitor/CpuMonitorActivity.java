...
public class CpuMonitorActivity extends AppCompatActivity implements ActivityCompat.OnRequestPermissionsResultCallback {
...
//	private ScriptIntrinsicBlur intrinsic;
	...
	private ServiceConnection mServiceConnection = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName className, IBinder service) {
			mSR = ((ReaderService.ServiceReaderDataBinder) service).getService();
			...
			mTVMemTotal.setText(mFormat.format(mSR.getMemTotal()) + Constants.kB);
			...
			if (tempIntent !=null) {
				tempIntent.putExtra(Constants.screenRotated, true);
	...
	private BroadcastReceiver receiverSetIconRecord = new BroadcastReceiver() {
		...
		@Override
		public void onReceive(Context context, Intent intent) {
			switchParameterForProcess((Map<String, Object>) intent.getSerializableExtra(Constants.process));
		}
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		intervalRead = mPrefs.getInt(Constants.intervalRead, Constants.defaultIntervalUpdate);
		...
		cpuTotal = mPrefs.getBoolean(Constants.cpuTotal, true);
		cpuAM = mPrefs.getBoolean(Constants.cpuAM, true);
		
		memUsed = mPrefs.getBoolean(Constants.memUsed, true);
		memAvailable = mPrefs.getBoolean(Constants.memAvailable, true);
		memFree = mPrefs.getBoolean(Constants.memFree, false);
		cached = mPrefs.getBoolean(Constants.cached, false);
		threshold = mPrefs.getBoolean(Constants.threshold, true);
...
//		sHeight = res.getDisplayMetrics().heightPixels;
		...
		statusBarHeight = res.getDimensionPixelSize(res.getIdentifier(Constants.sbh, Constants.dimen, Constants.android));
		
		final SeekBar mSBWidth = (SeekBar) findViewById(R.id.SBIntervalWidth);
		if (savedInstanceState != null && !savedInstanceState.isEmpty() && savedInstanceState.getInt(Constants.orientation) != orientation)
			orientationChanged = true;
		
		
		mVG = (ViewGraphic) findViewById(R.id.ANGraphic);
		
		graphicMode = mPrefs.getInt(Constants.graphicMode, Constants.graphicModeShowMemory);
		...
		mBHide.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				graphicMode = graphicMode == Constants.graphicModeShowMemory ? Constants.graphicModeHideMemory : Constants.graphicModeShowMemory;
				mPrefs.edit().putInt(Constants.graphicMode, graphicMode).apply();
				mVG.setGraphicMode(graphicMode);
				mBHide.setChecked(graphicMode == Constants.graphicModeShowMemory ? false : true);
				mHandlerVG.post(drawRunnableGraphic);
			}
		});
		mBHide.setChecked(graphicMode == Constants.graphicModeShowMemory ? false : true);
		
		processesMode = mPrefs.getInt(Constants.processesMode, Constants.processesModeShowCPU);
		...
		mBMemory.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				processesMode = processesMode == Constants.processesModeShowCPU ? Constants.processesModeShowMemory : Constants.processesModeShowCPU;
				mPrefs.edit().putInt(Constants.processesMode, processesMode).apply();
...
/*		} else {
		...
		mLButtonRecord.setOnLongClickListener(new View.OnLongClickListener() {
			...
			public boolean onLongClick(View arg0) {
				...
				Toast.makeText(CpuMonitorActivity.this, getString(id), Toast.LENGTH_SHORT).show();
				return true;
			}
		});
		...
		mLCPUTotal.setTag(Constants.cpuTotal);
		...
		mLCPUAM.setTag(Constants.cpuAM);
		...
		mLMemUsed.setTag(Constants.memUsed);
		...
		mLMemAvailable.setTag(Constants.memAvailable);
		...
		mLMemFree.setTag(Constants.memFree);
		...
		mLCached.setTag(Constants.cached);
		...
		mLThreshold.setTag(Constants.threshold);
...
//				mIVSettingsBG.getLayoutParams().height = settingsHeight;
		...
		mLGraphicSurface.setOnLongClickListener(new View.OnLongClickListener() {
			@Override
			public boolean onLongClick(View arg0) {
				Toast.makeText(CpuMonitorActivity.this, getString(R.string.menu_settings_description), Toast.LENGTH_SHORT).show();
				return true;
			}
		});
...
//				yBottom = (int) (getHeight()*0.88);
		...
		mBChooseProcess.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				Intent i = new Intent(CpuMonitorActivity.this, ProcessesActivity.class);
		...
		mCloseSettings.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (CpuMonitorActivity.this.intervalRead != intervalRead) {
					mSR.getCPUTotalP().clear();
					mSR.getCPUAMP().clear();
					
					if (mListSelected != null && !mListSelected.isEmpty())
						for (Map<String, Object> process : mListSelected) {
							process.put(Constants.pFinalValue, new ArrayList<Float>());
							process.put(Constants.pTPD, new ArrayList<Integer>());
						}
				...
				CpuMonitorActivity.this.intervalRead = intervalRead;
				CpuMonitorActivity.this.intervalUpdate = intervalUpdate;
				CpuMonitorActivity.this.intervalWidth = intervalWidth;
				...
				mHandler.post(drawRunnable);
				mPrefs.edit()
						.putInt(Constants.intervalRead, intervalRead)
						.putInt(Constants.intervalUpdate, intervalUpdate)
						.putInt(Constants.intervalWidth, intervalWidth)
						.apply();
			}
		});
		...
		if (Build.VERSION.SDK_INT >= 23 && checkSelfPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_DENIED)
			ActivityCompat.requestPermissions(this, new String[] { Manifest.permission.WRITE_EXTERNAL_STORAGE }, Constants.storagePermission);
		
		if (savedInstanceState != null && !savedInstanceState.isEmpty()) {
			processesMode = savedInstanceState.getInt(Constants.processesMode);
			mBMemory.setText(processesMode == Constants.processesModeShowCPU ? getString(R.string.w_main_memory) : getString(R.string.p_cpuusage));
			mVG.setProcessesMode(processesMode);
			
			canvasLocked = savedInstanceState.getBoolean(Constants.canvasLocked);
			settingsShown = savedInstanceState.getBoolean(Constants.settingsShown);
			if (settingsShown)
				mLSettings.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
					@Override
					public void onGlobalLayout() {
			...
			if (savedInstanceState.getBoolean(Constants.menuShown))
...
//		if (true) {
		if (mPrefs.getBoolean(Constants.welcome, true)) {
			mPrefs.edit().putLong(Constants.welcomeDate, Calendar.getInstance(TimeZone.getTimeZone(Constants.europeLondon)).getTimeInMillis()).apply();
			...
			if (v != null) { // This is to avoid a null pointer when the second time this code is executed (findViewById() returns the view only once)
				...
				(mLWelcome.findViewById(R.id.BHint)).setOnClickListener(new View.OnClickListener() {
					@Override
					public void onClick(View v) {
						mPrefs.edit().putBoolean(Constants.welcome, false).apply();
		...
		long time = Calendar.getInstance(TimeZone.getTimeZone(Constants.europeLondon)).getTimeInMillis();
		
//		if (true) {
		if (((float) (time - mPrefs.getLong(Constants.welcomeDate, 1)) / (24 * 60 * 60 * 1000) > 4
				&& mPrefs.getBoolean(Constants.feedbackFirstTime, true))
				|| ((float) (time - mPrefs.getLong(Constants.welcomeDate, 1)) / (24 * 60 * 60 * 1000) > 90)
				&& !mPrefs.getBoolean(Constants.feedbackDone, false)) {
			mPrefs.edit().putBoolean(Constants.feedbackFirstTime, false).apply();
			...
			if (v != null) { // This is to avoid a null pointer when the second time this code is executed (findViewById() returns the view only once)
				...
				(mLFeedback.findViewById(R.id.BFeedbackYes)).setOnClickListener(new View.OnClickListener() {
					@Override
					public void onClick(View v) {
						mPrefs.edit().putBoolean(Constants.feedbackDone, true).apply();
						try {
							startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(Constants.marketDetails + getPackageName()))
				...
				(mLFeedback.findViewById(R.id.BFeedbackDone)).setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						mPrefs.edit().putBoolean(Constants.feedbackDone, true).apply();
						Toast.makeText(CpuMonitorActivity.this, getString(R.string.w_main_feedback_done_thanks), Toast.LENGTH_SHORT).show();
						mLFeedback.animate().setDuration(animDuration).setListener(new AnimatorListenerAdapter() {
							@Override
							public void onAnimationEnd(Animator animation) {
								((ViewManager) mLFeedback.getParent()).removeView(mLFeedback);
				...
				(mLFeedback.findViewById(R.id.BFeedbackNo)).setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						...
						mPrefs.edit().putLong(Constants.welcomeDate, Calendar.getInstance(TimeZone.getTimeZone(Constants.europeLondon)).getTimeInMillis()).apply();
						Toast.makeText(CpuMonitorActivity.this, getString(R.string.w_main_feedback_no_remind), Toast.LENGTH_LONG).show();
					}
				});
...
//		mLSettings.animate().setDuration(animTime).setInterpolator(new AccelerateDecelerateInterpolator()).y(sHeight + navigationBarHeight);
	...
	private void switchParameter(boolean draw, LinearLayout labelRow) {
		...
		mPrefs.edit()
				.putBoolean(Constants.cpuTotal, cpuTotal)
				.putBoolean(Constants.cpuAM, cpuAM)
				
				.putBoolean(Constants.memUsed, memUsed)
				.putBoolean(Constants.memAvailable, memAvailable)
				.putBoolean(Constants.memFree, memFree)
				.putBoolean(Constants.cached, cached)
				.putBoolean(Constants.threshold, threshold)
	...
	private void switchParameterForProcess(Map<String, Object> process) {
		...
		for (int n=0; n<mLProcessContainer.getChildCount(); ++n) {
			...
			if (((Map<String, Object>) l.getTag()).get(Constants.pId).equals(process.get(Constants.pId)))
				break;
		}
		ImageView iv = (ImageView) l.getChildAt(0);
		
		if (process.get(Constants.pDead) != null) {
		...
		if ((Boolean) process.get(Constants.pSelected)) {
	...
	private void setTextLabelMemory(TextView absolute, TextView percent, List<String> values) {
		if (!values.isEmpty()) {
			...
			percent.setText(mFormatPercent.format(Integer.parseInt(values.get(0)) * 100 / (float) mSR.getMemTotal()) + Constants.percent);
		}
	}
	...
	private void setTextLabelCPUProcess(LinearLayout l) {
		...
		if (entry != null
				&& entry.get(Constants.pFinalValue) != null && ((List<String>) entry.get(Constants.pFinalValue)).size() != 0
				&& entry.get(Constants.pTPD) != null && !((List<String>) entry.get(Constants.pTPD)).isEmpty()
				&& entry.get(Constants.pDead) == null)
			if (processesMode == Constants.processesModeShowCPU)
				((TextView) l.findViewById(R.id.TVpPercentage)).setText(mFormatPercent.format(((List<String>) entry.get(Constants.pFinalValue)).get(0)) + Constants.percent);
			else ((TextView) l.findViewById(R.id.TVpPercentage)).setText(mFormatPercent.format(((List<Integer>) entry.get(Constants.pTPD)).get(0) * 100 / (float) mSR.getMemTotal()) + Constants.percent);
	}
	...
	private void setTextLabelMemoryProcesses(LinearLayout l) {
		...
		if (processesMode == Constants.processesModeShowCPU)
			...
			if (entry != null
				...
				tv.setText(mFormat.format(((List<String>) entry.get(Constants.pTPD)).get(0)) + Constants.kB);
			}
		}
	}
	...
	@SuppressLint({"NewApi", "InflateParams", "MissingSuperCall"})
	@SuppressWarnings("unchecked")
	@Override
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == 1 && resultCode == 1) {
			...
			if (data != null) {
				mListSelectedProv = (List<Map<String, Object>>) data.getSerializableExtra(Constants.listSelected);
				...
				for (Map<String, Object> process : mListSelectedProv) {
					process.put(Constants.pColour, getColourForProcess(mSR.getProcesses() != null ? mSR.getProcesses().size() : 0));
					mSR.addProcess(process);
				}

				mListSelected = mSR.getProcesses();

				if (data.getBooleanExtra(Constants.screenRotated, false))
			...
			synchronized (mListSelected) {
				for (final Map<String, Object> process : mListSelectedProv) {
					if (process.get(Constants.pSelected) == null)
					...
					l.setOnClickListener(new View.OnClickListener() {
						@Override
						public void onClick(View v) {
							Boolean b = (Boolean) process.get(Constants.pSelected);
							process.put(Constants.pSelected, !b);
					...
					Drawable d = null;
					try {
						d = getPackageManager().getApplicationIcon((String) process.get(Constants.pPackage));
					...
					int colour = (Integer) process.get(Constants.pColour);

					TextView pName = (TextView) l.findViewById(R.id.TVpAppName);
					pName.setText((String) process.get(Constants.pAppName));
					pName.setTextColor(colour);

					TextView pId = (TextView) l.findViewById(R.id.TVpName);
					pId.setText("Pid: " + process.get(Constants.pId));
...
//					if (true) {
					if (mPrefs.getBoolean(Constants.firstTimeProcesses, true)) {
						...
						if (v != null) { // This is to avoid a null pointer when the second time this code is executed (findViewById() returns the view only once)
							...
							mLWelcome.findViewById(R.id.BHint).setOnClickListener(new View.OnClickListener() {
								@Override
								public void onClick(View v) {
									mPrefs.edit().putBoolean(Constants.firstTimeProcesses, false).apply();
									mLWelcome.animate().setDuration(animDuration).setListener(new AnimatorListenerAdapter() {
										@Override
										public void onAnimationEnd(Animator animation) {
											((ViewManager) mLWelcome.getParent()).removeView(mLWelcome);
	...
	public void onSaveInstanceState(Bundle outState) {
		outState.putInt(Constants.orientation, orientation);
		outState.putBoolean(Constants.menuShown, mPWMenu.isShowing());
		outState.putBoolean(Constants.settingsShown, settingsShown);
		outState.putBoolean(Constants.canvasLocked, canvasLocked);
	}
	...
	public void onStart() {
		...
		registerReceiver(receiverSetIconRecord, new IntentFilter(Constants.actionSetIconRecord));
		registerReceiver(receiverDeadProcess, new IntentFilter(Constants.actionDeadProcess));
	...
	public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
		if (requestCode == Constants.storagePermission && PackageManager.PERMISSION_DENIED == grantResults[0]) {
			Toast.makeText(CpuMonitorActivity.this, getString(R.string.w_main_storage_permission), Toast.LENGTH_LONG).show();
		}
	}
}
